...
def tstamp():
    ...
    return current_time
#added timestamp function for prints
...
def snipe(recv_time,snipe_delay,channelid):
    if snipe_delay != 0.0:
        try:
            ...
            return
    if int(channelid) == 886986837363667014:
        time.sleep(2.6)
        return print(f"{int(channelid)} waited 3 secs")                  
    if int(channelid) != 886986837363667014:
        time.sleep(0.3)
        return print(f"{int(channelid)} waited 0.5 secs")
    time.sleep(0.2)
        ...
        if c_settings['pending'] == None and int(aId) != mudae and content[0:c_settings['prefix_len']] == c_settings['prefix'] and content.split(' ')[0][c_settings['prefix_len']:] in mudae_cmds:
            ...
            if waifu_wall.get(channelid,0) != next_claim(channelid)[0]:
                ...
                if str(user['id']) in content:
                    ...
                    snipe(recv,snipe_delay,channelid)                
                    ...
                    if "reactions" in m_reacts:
                        ...
                        bot.addReaction(channelid, messageid, "🆓")#replace these emotes with your own
                        time.sleep(2)
                ...
                if charname.lower() in chars:
                    ...
                    if "reactions" in m_reacts:
                        ...
                        bot.addReaction(channelid, messageid, "🆓")#replace all these emotes with your own
                        time.sleep(2)
                        bot.sendMessage(channelid, ":0") 
        ...
        if reactionid == mudae and int(rchannelid) in mhids:
            ...
            if emojiid == None:
                if emoji in eventlist:
                    ...
                    time.sleep(2+snipe_delay) #edit this for event reaction time
